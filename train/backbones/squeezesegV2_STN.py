...
class SqueezesegImageEncoder(nn.Module):
  ...
  def __init__(self, params):
    ...
    self.conv1a = nn.Sequential(nn.Conv2d(self.input_depth, 64, kernel_size=3,
                                          stride=[self.strides[0], self.strides[0]],
                                          padding=1),
    ...
    self.fire23 = nn.Sequential(nn.MaxPool2d(kernel_size=3,
                                             stride=[self.strides[1], self.strides[1]],
                                             padding=1),
...
class STN(nn.Module):
  """
  class for spatial transformer
  modifed copy from: https://pytorch.org/tutorials/intermediate/spatial_transformer_tutorial.html
  """
  def __init__(self, params):
    super(STN, self).__init__()
    self.bn_d = params["bn_d"]
    self.image_encoder_channels = params["image_encoder_channels"]
    outplane = 4
    self.localization = nn.Sequential(
      nn.Conv2d(self.image_encoder_channels, outplane, kernel_size=3, stride=2, padding=1),
      nn.LeakyReLU(True),
      nn.BatchNorm2d(outplane, momentum=self.bn_d),

      nn.Conv2d(outplane, outplane*2, kernel_size=3, stride=2, padding=1),
      nn.LeakyReLU(True),
      nn.BatchNorm2d(outplane*2, momentum=self.bn_d),

      nn.Conv2d(outplane*2, outplane*4, kernel_size=3, stride=2, padding=1),
      nn.LeakyReLU(True),
      nn.BatchNorm2d(outplane*4, momentum=self.bn_d),

      nn.Conv2d(outplane*4, 32, kernel_size=3, stride=2, padding=1),
      nn.LeakyReLU(True),
      nn.BatchNorm2d(32, momentum=self.bn_d)

    )
    self.head = nn.Sequential(OrderedDict([
      ("input", nn.AdaptiveAvgPool2d(output_size=1)),
      ("flatten", Flatten()),
      ("out", nn.Linear(32, 3 * 2 ))
    ]))
    
    # Initialize the weights/bias with identity transformation
    self.head._modules["out"].weight.data.zero_()
    self.head._modules["out"].bias.data.copy_(torch.tensor([1, 0, 0, 0, 1, 0], dtype=torch.float))

  def forward(self, x, lidar_size):
    xs = self.localization(x)
    theta = self.head(xs)
    theta = theta.view(-1, 2, 3)
    output_size = (x.size(0),x.size(1), lidar_size[2], lidar_size[3])
    print(output_size)
    grid = F.affine_grid(theta,  size=output_size)
...
class Backbone(nn.Module):
  ...
  def __init__(self, params):
    ...
    params["image_encoder_channels"] = self.image_encoder.get_last_depth()
  ...
  def forward(self, lidar, camera):
    ...
    print(x.shape)
    image_features = self.stn(image_features, x.size())
    print(image_features.shape)
