...
class BaseTrainer():
  ...
  def create_logger(self, hparams: list = None, metrics: list = None):
        # put logger where it belongs
        self.tb_logger = Logger(self.log + "/tb", hparams, metrics)
        return self
  ...
  def create_optimzer_parameter_groups(self):
  ...
  def make_log_img(depth, mask, pred, gt, color_fn):
    ...
    out_img = cv2.applyColorMap(
        depth, BaseTrainer.get_mpl_colormap('viridis')) * mask[..., None]
  ...
  def _train(self):
    ...
    for epoch in range(self.ARCH["train"]["max_epochs"]):
      ...
      # train for 1 epoch
      acc, iou, loss, update_mean = self._train_epoch(train_loader=self.parser.get_train_set(),
                                                     model=self.model,
      ...
      if epoch % self.ARCH["train"]["report_epoch"] == 0:
        ...
        print("*" * 80)
        acc, iou, loss, rand_img = self._validate(val_loader=self.parser.get_valid_set(),
                                                 model=self.model,
                                                 criterion=self.criterion,
                                                 evaluator=self.evaluator,
        ...
        # save to log
        BaseTrainer.save_to_log(logdir=self.log,
  ...
  def _train_epoch(self, train_loader, model, criterion, optimizer, epoch, evaluator, scheduler, color_fn, report=10, show_scans=False):
    ...
    for i, item in enumerate(train_loader):
      ...
      if show_scans:
        ...
        out = BaseTrainer.make_log_img(depth_np, mask_np, pred_np, gt_np, color_fn)
  ...
  def _validate(self, val_loader, model, criterion, evaluator, class_func, color_fn, save_scans):
    batch_time = AverageMeter()
    losses = AverageMeter()
    acc = AverageMeter()
    iou = AverageMeter()
    ...
    with torch.no_grad():
      ...
      for i, item in enumerate(val_loader):
        ...
        if save_scans:
          ...
          pred_np = argmax[0].cpu().numpy()
          gt_np = proj_labels[0].cpu().numpy()
          out = BaseTrainer.make_log_img(depth_np,
                                     mask_np,
                                     pred_np,
                                     gt_np,
                                     color_fn)
